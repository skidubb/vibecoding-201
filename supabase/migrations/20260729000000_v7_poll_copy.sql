...
--
-- Wording is verbatim from `deck-content-v7.md`, including the option text. The
-- room looks at the deck and the site at the same time; a paraphrase on one of
-- them is a distraction at exactly the wrong moment.
--
-- Correct answers are unchanged (C, B, C) and stay behind the column grant from
-- `20260727002000_revoke_default_grants.sql`. States are left alone: a poll that
-- is open when this runs stays open, because re-closing a live poll mid-class
-- would be a worse failure than stale copy.
...
update public.polls set
  ...
  -- "and test a fix" is the 201 addition. The room learned the diagnose loop in
  -- 101; testing the fix is what stops the error recurring, and it is the whole
  -- reason this poll is still in the deck.
